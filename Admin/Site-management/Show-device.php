...
@$Dave = $_GET['T'];
if (@$_GET['d'] == 'd') {
    $ves = "DELETE FROM devices WHERE Devices_token='$Dave'";
...
@$upd = $_GET['T'];
$edi = "SELECT * FROM devices WHERE Devices_token='$upd'";
...
if (isset($_POST['get04'])) {
    if (empty($post01) || empty($post02) || empty($post03) || empty($image)) {
        ...
        $Ces = "UPDATE devices SET
                Devices_name = '$post01',
                Devices_info = '$post02',
                Devices_img = '$Vis',
                Devices_price = '$post03'
                WHERE Devices_token = '$upd'
            ";
        if (mysqli_query($shoes, $Ces)) {
            echo "
           
                <meta http-equiv='refresh' content='0; url=Show-device.php?T=' . $upd . ''/>
            ";
            exit();
        }
    }
            ...
            <div class="container">
                ...
                <ul class="responsive-table">
                    <li class="table-header">
                        ...
                        <div class="col col-1">الموصفات</div>
                ...
                <div class="col col-1" data-label="الصورة"><img class="style-img" src="IMG-AAdmin/' . $Runshow['Devices_img'] . '" /></div>
                <div class="col col-1" data-label="الاسم">' . $Runshow['Devices_name'] . '</div>
                <div class="col col-1" data-label="الموصفات">' . $Runshow['Devices_info'] . '</div>
                <div class="col col-1" data-label="السعر">' . $Runshow['Devices_price'] . '</div>
                <div class="col col-1" data-label="تعديل">
                    <a href="Show-device.php?edit=' . $Runshow['Devices_token'] . '">تعديل</a>
                </div>
                <div class="col col-1" data-label="حذف">
                    <a href="Show-device.php?d=d&T=' . $Runshow['Devices_token'] . '" class="Runshow"><i class="fa-solid fa-trash" style="color:#ff9f0d;"></i></a>
                </div>
            </li>';
                    }
                    ?>
                ...
                <?php if (isset($_GET['edit'])) : ?>
                    ...
                    $deviceQuery = "SELECT * FROM devices WHERE Devices_token='$deviceToken'";
                    ...
                    <div id="editModal" class="modal">
                        <div class="modal-content">
                            ...
                            <h4>تعديل الجهاز</h4>
                            <form id="editForm" action="Show-device.php?T=<?php echo $device['Devices_token']; ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="editToken" value="<?php echo $device['Devices_token']; ?>">
                                <div class="form-group">
                                    <label for="editProductName">الاسم</label>
                                    <input type="text" class="form-control" name="get01" value="<?php echo $device['Devices_name']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editIngredients">الموصفات</label>
                                    <input type="text" class="form-control" name="get02" value="<?php echo $device['Devices_info']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editInstructions">السعر</label>
                                    <input type="text" class="form-control" name="get03" value="<?php echo $device['Devices_price']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editImage">الصورة الحالية</label><br>
                                    <img src="IMG-AAdmin/<?php echo $device['Devices_img']; ?>" alt="صورة الجهاز" style="width: 30%;"><br>
