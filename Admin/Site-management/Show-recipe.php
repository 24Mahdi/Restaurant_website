...
@$Dave = $_GET['T'];
if (@$_GET['d'] == 'd') {
    $ves = "DELETE FROM recipe WHERE recipe_token='$Dave'";
...
@$upd = $_GET['T'];
$edi = "SELECT * FROM recipe WHERE recipe_token='$upd'";
...
if (isset($_POST['get04'])) {
    if (empty($post01) || empty($post02) || empty($post03) || empty($image)) {
        ...
        $Ces = "UPDATE recipe SET
                recipe_name = '$post01',
                recipe_ingredients = '$post02',
                recipe_img = '$Vis',
                recipe_info = '$post03'
                WHERE recipe_token = '$upd'
            ";
        if (mysqli_query($shoes, $Ces)) {
            echo "
                <meta http-equiv='refresh' content='0; url=Show-recipe.php?T=' . $upd . ''/>
            ";
            exit();
        }
    }
            ...
            <div class="container">
                ...
                <ul class="responsive-table">
                    <li class="table-header">
                        ...
                        <div class="col col-1">المكونات</div>
                        <div class="col col-1">طريقة التحضير</div>
                ...
                <div class="col col-1" data-label="الصورة"><img class="style-img" src="IMG-AAdmin/' . $Runshow['recipe_img'] . '" /></div>
                <div class="col col-1" data-label="الاسم">' . $Runshow['recipe_name'] . '</div>
                <div class="col col-1" data-label="المكونات">' . $Runshow['recipe_ingredients'] . '</div>
                <div class="col col-1" data-label="طريقة التحضير">' . $Runshow['recipe_info'] . '</div>
                <div class="col col-1" data-label="تعديل">
                    <a href="Show-recipe.php?edit=' . $Runshow['recipe_token'] . '">تعديل</a>
                </div>
                <div class="col col-1" data-label="حذف">
                    <a href="Show-recipe.php?d=d&T=' . $Runshow['recipe_token'] . '" class="Runshow"><i class="fa-solid fa-trash" style="color:#ff9f0d;"></i></a>
                </div>
            </li>';
                    }
                    ?>
                ...
                <?php if (isset($_GET['edit'])) : ?>
                    ...
                    $deviceQuery = "SELECT * FROM recipe WHERE recipe_token='$deviceToken'";
                    ...
                    <div id="editModal" class="modal">
                        <div class="modal-content">
                            ...
                            <h4>تعديل الوصفة</h4>
                            <form id="editForm" action="Show-recipe.php?T=<?php echo $device['recipe_token']; ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="editToken" value="<?php echo $device['recipe_token']; ?>">
                                <div class="form-group">
                                    <label for="editProductName">الاسم</label>
                                    <input type="text" class="form-control" name="get01" value="<?php echo $device['recipe_name']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editIngredients">المكونات</label>
                                    <input type="text" class="form-control" name="get02" value="<?php echo $device['recipe_ingredients']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editInstructions">طريقة التحضير</label>
                                    <input type="text" class="form-control" name="get03" value="<?php echo $device['recipe_info']; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="editImage">الصورة الحالية</label><br>
                                    <img src="IMG-AAdmin/<?php echo $device['recipe_img']; ?>" alt="صورة الجهاز" style="width: 30%;"><br>
